...
@section('content')
<form id="formAuthentication" class="mb-3" action="{{url('/settings-list')}}" method="POST">
    @csrf
    <div class="card">
        <div class="row gy-4">
            ...
            <div class="col-md-12 col-lg-4">
                <div class="card-body">
                    ...
                    <div class="mt-2 mb-3">
                        ...
                        <select id="exam_type" name="exam_type" class="form-select form-select-lg">
    ...
    <div class="table-responsive text-nowrap">
        <table class="table">
            ...
            <tbody class="table-border-bottom-0">
                @foreach($settings_data as $sett)

                    <tr>
                        <td>{{$loop->iteration}}</td>
                        </td>
                        <td>{{$sett['class']}}</td>
                        <td>{{$sett['min_marks']}}</td>
                        <td>{{$sett['max_marks']}}</td>
                        <td>{{$sett['extra_qp_count']}}</td>
                        <td>{{$sett['duration']}}</td>
                        <td>{{$sett['re_exam']}}</td>
                    </tr>
                @endforeach
