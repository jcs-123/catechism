...
@section('content')
    ...
    @endif
    <h5 class="mt-4 mb-3">STUDENTS ADDRESS LIST </h5>

    <form id="formAuthentication" class="mb-3" action="{{url('/staff-address-report')}}" method="POST">
        @csrf
        <div class="card p-3">
            <div class="row gy-4">
    ...
    <div class="card">
        <h5 class="card-header">STAFF ADDRESS List</h5>
            ...
            @foreach($address_list as $data)
                ...
                <div class="col-md-12 col-xl-4">
                    <div class="card shadow-none bg-transparent border border-primary mb-3">
                        <div class="card-body">
                            <h5 class="card-title">{{$data['staff_name']}}</h5>
                            <p class="card-text">
                                {{$data['permanent_address']}}</br>{{$data['post']}}
                                </br>{{$data['place']}}
                                </br>{{$data['pin_code']}}
                            </p>
                        </div>
