...
@section('content')
<form id="formAuthentication" class="mb-3" action="{{url('/cancel-tc-list')}}" method="POST">
    @csrf
    <div class="card">
        <div class="row gy-4">
            @if(session('success'))
    ...
    <div class="table-responsive text-nowrap">
        <table class="table">
            ...
            <tbody class="table-border-bottom-0">
                @if(isset($transfered_staffs) && !empty($transfered_staffs))
                    @foreach($transfered_staffs as $staffs)

                        <tr>
                            <td>{{$loop->iteration}}</td>
                            </td>
                            <td>{{$staffs['to_unit_name']}}</td>
                            <td>{{$staffs['staff_name']}}</td>
                            <td>{{$staffs['transfer_reason']}}</td>
                            <td>
                                <a href="{{ route('cancelTC', ['transfer_id' => $staffs['transfer_id']]) }}">Cancel TC</a>
                            </td>
                            <td></td>
                        </tr>
                    @endforeach
